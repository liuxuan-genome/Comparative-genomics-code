...
if ( $ProcessFile eq "" || !( $ProcessFile=~/^\d+$/) ) {
    print "Required parameter for processor number.\n";
    print "For help information: $0 -help.\n";
    exit(0);
}
...
if (-e $OutputFolder) {
	print " outputdir is already exists and will be removed.\n creating the folder $OutputFolder, \n";
	system("rm -rf $OutputFolder");
	mkdir($OutputFolder);
	#exit(0);
}
else {
...
sub RunMultiProcesses {
	...
	while (@Commands) {
        ...
        $num=$ProcessFile; #huangls 2022.12.30
	...
	for (my $i=0; $i<@homologs; $i++) {
	  ...
	  #print "@genes\n";	
		...
		for (my $j=0; $j<@genes && $flag; $j++) {
			...
			if (exists $aa_seqs{$genes[$j]} && exists $nuc_seqs{$genes[$j]}) {
				$aa_seq .= ">".$genes[$j]."\n";
			  ...
			  print("\n homolog gene ID : $genes[$j] not in cds or pep fasta files, please check ,exit\n");
			  exit(1);
				$flag = 0;
			}
		}
...
sub AlignProteinSeqs(){
	...
	if ($MSACMD =~ /clustalw2/) {
	    ...
	    $cmd = "muscle -quiet  -in ".$file.".pep  -out ".$file.".pep_aln >> msg.msa";  #huangls 
	}
